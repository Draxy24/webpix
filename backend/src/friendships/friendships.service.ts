...
export class FriendshipsService {
  ...
  async sendRequest(senderId: number, receiverNickname: string) {
    const receiver = await this.prisma.user.findUnique({ where: { nickname: receiverNickname } });
    if (!receiver) throw new NotFoundException('Usuario no encontrado');
    if (receiver.id === senderId) throw new BadRequestException('No puedes enviarte solicitud a ti mismo');
  ...
  async accept(friendshipId: number, userId: number) {
    ...
    if (friendship.receiverId !== userId) throw new ForbiddenException('No puedes aceptar esta solicitud');
    if (friendship.status !== 'PENDING') throw new BadRequestException('Esta solicitud ya fue procesada');
  ...
  async decline(friendshipId: number, userId: number) {
    const friendship = await this.prisma.friendship.findUnique({ where: { id: friendshipId } });
    if (!friendship) throw new NotFoundException('Solicitud no encontrada');
    if (friendship.receiverId !== userId) throw new ForbiddenException('No puedes declinar esta solicitud');
    if (friendship.status !== 'PENDING') throw new BadRequestException('Esta solicitud ya fue procesada');
  ...
  async listIncoming(userId: number) {
    ...
    return requests.map((r) => ({ friendshipId: r.id, sender: r.sender, createdAt: r.createdAt }));
  }

  async listOutgoing(userId: number) {
    const requests = await this.prisma.friendship.findMany({
      where: { senderId: userId, status: 'PENDING' },
      include: { receiver: { select: { id: true, nickname: true, profilePic: true } } },
    });
    return requests.map((r) => ({ friendshipId: r.id, receiver: r.receiver, createdAt: r.createdAt }));
  }

  async getStatus(userId: number, otherNickname: string) {
    const other = await this.prisma.user.findUnique({ where: { nickname: otherNickname } });
    ...
    if (friendship.status === 'PENDING') {
      ...
      return { status: isFromMe ? 'REQUEST_SENT' : 'REQUEST_RECEIVED', friendshipId: friendship.id };
    }
    return { status: 'NONE' };
  }
}